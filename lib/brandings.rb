...
class Brandings
  ...
  def self.check_login(token, url)
    Brandings.set_headers(token)
    response = get(Brandings.root_url(url)+"/chatter/users/me/")
  end

  def self.my_info(token, url)
    Brandings.set_headers(token)
    response = get(Brandings.root_url(url)+"/chatter/users/me/")
  end
  
  def self.find(token, url)
    Brandings.set_headers(token)
    response = get(Brandings.root_url(url)+"/chatter/users/me/")
    response = get(Brandings.root_url(url)+"/sobjects/User/"+response["id"])
  ...
  def self.find_products(token, url)
    Brandings.set_headers(token)
